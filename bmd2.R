...
bmd <- function (X, Y, alpha = 0.05, OL_thres = 0.9, tag = NULL, saveDir = NULL, cp_cor = TRUE,
  ...
  pvalue <- function(A, test_x){
    if (test_x) {
      otherMat <- Y_scaled[ , A, drop = FALSE]
      cross <- cross_cor(Y_indx = A)
      thisMat <- X_scaled
    } else {
      otherMat <- X_scaled[, A, drop=FALSE]
      cross <- t(cross_cor(X_indx=A))
      thisMat <- Y_scaled
    }
    # Calculating the variances
    {
        this4 <- colSums(thisMat^4)
        xRowSum <- rowSums(otherMat)
        xRowSum2 <- tcrossprod(cross, otherMat^2)
        
        # Calc for star 1
        star1 <- crossprod(thisMat^2, xRowSum^2)
        
        # Calc for star 2
        star2 <- this4 * rowSums(cross)^2
        
        # Calc for star 3
        star3 <- 2 * rowSums(cross) * colSums(thisMat^2 * t(xRowSum2))
        
        # Calc for star 4
        star4 <- rowSums(xRowSum2^2)
        
        # Calc for dagger 1
        dagger1 <- rowSums(cross) * crossprod(thisMat^3, xRowSum)
        
        # Calc for dagger 2
        dagger2 <- colSums(xRowSum * t(xRowSum2) * thisMat)
        
        allvars <- (star1 + 0.25 * (star2 + star3 + star4) - dagger1 - dagger2) / (n - 1)
      }
    
    corsums <- as.vector(rowSums(cross))
  ...
  # Getting node orders
  Ysum <- rowSums(Y_scaled) 
  Xsum <- rowSums(X_scaled)
  cor_X_to_Ysums <- crossprod(Ysum, X_scaled)
  cor_Y_to_Xsums <- crossprod(Xsum, Y_scaled)
  ...
  while (length(c(remainingX, remainingY)) > 0) {
    ...
    repeat {
      ...
      if (updateMethod != 7) {
        ...
        if (comm_indx > dx) {
          Xpvals <- pvalue(B_oldy, TRUE)
          Ypvals <- pvalue(B_oldx, FALSE)
          B_new <- bh_reject(c(Xpvals, Ypvals), alpha)
          B_newx <- B_new[B_new <= dx]
          B_newy <- B_new[B_new > dx]
        } else {
          Xpvals <- pvalue(B_oldx, FALSE)
          Ypvals <- pvalue(B_oldy, TRUE)
          B_new <- bh_reject(c(Xpvals, Ypvals), alpha)
          B_newx <- B_new[B_new > dx]
          B_newy <- B_new[B_new <= dx]
        }
